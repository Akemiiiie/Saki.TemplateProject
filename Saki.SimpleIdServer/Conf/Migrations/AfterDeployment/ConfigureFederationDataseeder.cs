...
namespace Saki.SimpleIdServer.Conf.Migrations.AfterDeployment
{
    public class ConfigureFederationDataseeder : BaseAfterDeploymentDataSeeder
    {
        ...
        protected override async Task Execute(CancellationToken cancellationToken)
        {
            ...
            {
                var existingScope = await _scopeRepository.GetByName(DefaultRealms.Master.Name, ConfigureIdServerFederationDataseeder.FederationEntitiesScope.Name, cancellationToken);
                var existingClient = await _clientRepository.GetByClientId(DefaultRealms.Master.Name, DefaultClients.SidAdminClientId, cancellationToken);
