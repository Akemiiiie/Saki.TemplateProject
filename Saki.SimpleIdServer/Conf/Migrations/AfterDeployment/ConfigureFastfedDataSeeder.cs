...
namespace Saki.SimpleIdServer.Conf.Migrations.AfterDeployment
{
    public class ConfigureFastfedDataSeeder : BaseAfterDeploymentDataSeeder
    {
        ...
        protected override async Task Execute(CancellationToken cancellationToken)
        {
            ...
            {
                var masterRealm = await _realmRepository.Get(DefaultRealms.Master.Name, cancellationToken);
                var adminUser = await _userRepository.GetBySubject(DefaultUsers.Administrator.Name, DefaultRealms.Master.Name, cancellationToken);
                ...
                var existingScopes = await _scopeRepository.GetByNames(DefaultRealms.Master.Name, scopeNames, cancellationToken);
                var existingGroups = await _groupRepository.GetAllByStrictFullPath(DefaultRealms.Master.Name, AllGroups.Select(g => g.FullPath).ToList(), cancellationToken);
                var existingClients = await _clientRepository.GetByClientIds(DefaultRealms.Master.Name, clientIds, cancellationToken);
